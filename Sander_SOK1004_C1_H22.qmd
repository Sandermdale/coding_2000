...
bnp%>%
  mutate(per_tusen = verdi/1000)%>%
  filter(tid >= 2000)%>%
  ggplot(aes(x=tid, y=per_tusen, col=var))+
  geom_line()+
  theme_bw()
```
...
som vi kan se i grafen har grafen som inneholder data fra bruttonasjonalprodukt hvor prisene øker forholdsvis til resten av verden og vi får derfor en graf som har en veldig stor økning. men derimot med grafen til de med fast pris er det en mer jevn økning for de bruker samme priser hvert år. grunnen for at de ikke har lik økning er for aller priser er blitt dyrere og det er blitt dyrer å drive/lage produkter og etc. med de faste prisene blir ikke grafen påvirket av f.eks finanskrisen og vil ikke være en realistik graf for å vise hvordan BNP er hver enkelt år. men det grafen med faste priser viser oss er hvordan f.eks 2000 er sammenlignet med prisne vi hadde i 2015 slik vi kan se hvilket år ville vært det "beste".
...
Med denne koden laget vi en nytt datasett under df_wide som gjorde at de tre kollonnene var BNP, tid og BNP med faste priser. med hjelp av denne koden har vi også gjort tabellen mer ryddig enn hvis vi skal sammenlignet med datataen i df. i df_wide har vi valgt fakotrene BNP, BNP faste priser og tid noe som gjør den mer oversiktlig og lettere og lese den, men i DF har vi tid verdi og statistikkvariabell.
...
# Besvar oppgave IIc 
bnp_wide <- bnp_wide%>%
  relocate("MEMO: Bruttonasjonalprodukt. Faste 2015-priser", .before= "LBNP")
bnp_long <- bnp_wide%>%
...
#ja det er negativ vekst på grunn av prisøkninger på varer og økning av priser på produksjon. dette er noe du kan se i denne figuren.
